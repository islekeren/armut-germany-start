...
export async function apiRequest<T>(
  ...
  const { token, body, headers: incomingHeaders, ...fetchOptions } = options;
...
export const providersApi = {
  getAll: (query?: {
    ...
    if (query?.minRating !== undefined) params.append("minRating", String(query.minRating));
    ...
    return apiRequest<ProvidersResponse>(`/providers${queryString ? `?${queryString}` : ""}`);
  },
...
export const quotesApi = {
  ...
  getMyQuotes: (token: string) => apiRequest<Quote[]>("/quotes/my-quotes", { token }),

  getReceivedQuotes: (token: string) => apiRequest<Quote[]>("/quotes/received", { token }),

  getById: (token: string, id: string) => apiRequest<Quote>(`/quotes/${id}`, { token }),
